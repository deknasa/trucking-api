        ...
        if ($getcabang == 'MAKASSAR') {
            $query = DB::table($temprekapall)->from(
                ...
                ->select(
                    ...
                    db::raw("(case when (row_number() Over(partition BY (case when isnull(b.keterangan,'')='' then a.namabarang else isnull(b.keterangan,'') end) Order By (case when isnull(b.keterangan,'')='' then a.namabarang else isnull(b.keterangan,''),a.tglbukti,a.tglinput))=1 then a.qtysaldo else 0 end) as qtysaldo"),
                    db::raw("(case when (row_number() Over(partition BY (case when isnull(b.keterangan,'')='' then a.namabarang else isnull(b.keterangan,'') end) Order By (case when isnull(b.keterangan,'')='' then a.namabarang else isnull(b.keterangan,''),a.tglbukti,a.tglinput))=1 then a.nilaisaldo else 0 end) as nominalsaldo"),
