...
class Aco extends MyModel
{
    ...
    public function getUserAcl()
    {
        ...
        if ($proses == 'reload') {
            $temtabel = 'tempuseracl' . rand(1, getrandmax()) . str_replace('.', '', microtime(true)) . request()->nd ?? 0;
    ...
    public function getdataUserAcl($user_id)
    {
                    ...
                    foreach ($this->params['filters']['rules'] as $index => $filters) {
                        if ($filters['field'] != '') {
                            if ($filters['field'] == 'status') {
                                $query = $query->where('a.' . $filters['field'], '=', "$filters[data]");
