...
class PengeluaranHeader extends MyModel
{
    ...
    public function get()
    {
        ...
        if ($proses == 'reload') {
            ...
            Schema::create($temtabel, function (Blueprint $table) {
                ...
                $table->string('nobukti_asal', 50)->nullable();
            ...
            // KAS GANTUNG
            $getDataLain = DB::table("kasgantungheader")->from(DB::raw("kasgantungheader as a with (readuncommitted)"))
            ...
            // ABSENSI APPROVAL
            $url = config('app.url_fe') . 'absensisupirapprovalheader';
            ...
            // PELUNASAN HUTANG
            $url = config('app.url_fe') . 'pelunasanhutangheader';
            ...
            // PROSES GAJI SUPIR
            $url = config('app.url_fe') . 'prosesgajisupirheader';
            ...
            // PENDAPATAN SUPIR
            $url = config('app.url_fe') . 'pendapatansupirheader';
            ...
            $query = DB::table($this->table)->from(DB::raw("pengeluaranheader with (readuncommitted)"))
                ...
                ->leftJoin(DB::raw("$tempTable as asal with (readuncommitted)"), 'pengeluaranheader.nobukti', 'asal.nobukti')
    ...
    public function processStore(array $data): PengeluaranHeader
    {
        ...
        if ($statusreimbursement == $idreimbursement) {
            ...
            $tujuan_id=db::table("cabang")->from(db::raw("cabang a with (readuncommitted)"))
            ->select(
                'b.id'
            )
            ->join(db::raw("tujuan b with (readuncommitted)"),'a.namacabang','b.keterangan')
            ...
            $pelanggan = DB::table("cabang")->from(DB::raw("cabang with (readuncommitted)"))
                ...
                ->first()->pelanggan_id ?? '';
            $statusjenisbiaya=$data['statusjenisbiaya'] ?? 0;
            $invoiceEmklRequest = [
                'prosesreimburse' => 1,
                ...
                'statusjenisbiaya' =>   $statusjenisbiaya,
                'statusppn' => 0,
                'tgldari' => '',
                'tglsampai' => '',
