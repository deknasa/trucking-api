    ...
    public function store(StorePelangganRequest $request): JsonResponse
    {
        ...
        try {
            $data = [
                ...
                'npwp' => $request->npwp ?? '',
                'noktp' => $request->noktp ?? '',
                'alamatfakturpajak' => $request->alamatfakturpajak ?? '',
                'alamatkantorpenagihan' => $request->alamatkantorpenagihan ?? '',
                'namapemilik' => $request->namapemilik ?? '',
                'telpkantor' => $request->telpkantor ?? '',
                'faxkantor' => $request->faxkantor ?? '',
                'website' => $request->website ?? '',
                'contactperson' => $request->contactperson ?? '',
                'telpcp' => $request->telpcp ?? '',
                'statusasuransi' => $request->statusasuransi ?? 810,

                // 'asuransitas' => $request->asuransitas ?? '',
                // 'asuransisendiri' => $request->asuransisendiri ?? '',
                'top' => $request->top ?? 0,
                'prosedurpenagihan' => $request->prosedurpenagihan ?? '',
                'syaratpenagihan' => $request->syaratpenagihan ?? '',
                'pickeuangan' => $request->pickeuangan ?? '',
                'telppickeuangan' => $request->telppickeuangan ?? '',
                'jenisusaha' => $request->jenisusaha ?? '',
                'volumeperbulan' => $request->volumeperbulan ?? '',
                'kompetitor' => $request->kompetitor ?? '',
                'referensi' => $request->referensi ?? '',
                'nominalplafon' => $request->nominalplafon ?? 0,
                'danaditransferdari' => $request->danaditransferdari ?? '',
